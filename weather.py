...
while True:
        ...
        if precipitation_type == "rain":
            weather_info = tkntr.messagebox.showinfo(title=f"Weather {user_location}",
                                                  message=f"{user_location}\n\n"
                                                          f"Temperature: {temperature_now} K\n"
                                                          f"Feels Like: {temperature_feels_like} K\n"
    ...
    if choice == '2':
        location_window = tkntr.Tk()
        location_window.withdraw()
        user_location = simpledialog.askstring(title="Three Hours Forecast",
                                               prompt="Location:",
                                               initialvalue="City, Country")
        if user_location == "City, Country":
            user_location = "London, GB"
        ...
        if hourly_precipitation_type == "rain":
            weather_info = tkntr.messagebox.showinfo(title=f"Three Hours Forecast {user_location}",
                                                  message=f"Three Hours Forecast\n\n {user_location}\n\n"
                                                          f"Temperature: {temperature_per_hour} K\n"
                                                          f"Feels Like: {feels_like_per_hour} K\n"
